...
function MediaUnlockTest_UMAMUSUME() {
    echo -n -e " Cygame ウマ娘 プリティーダービー:\t->\c";
    # 测试，连续请求两次 (单独请求一次可能会返回35, 第二次开始变成0)
    ...
    if [ "$result" = "000" ]; then
        echo -n -e "\r Cygame ウマ娘 プリティーダービー:\t${Font_Red}Failed (Network Connection)${Font_Suffix}\n" && echo -e " Cygame ウマ娘 プリティーダービー:\tFailed (Network Connection)" >> ${LOG_FILE};
        elif [ "$result" = "404" ]; then
        echo -n -e "\r Cygame ウマ娘 プリティーダービー:\t${Font_Green}Yes${Font_Suffix}\n" && echo -e " Cygame ウマ娘 プリティーダービー:\tYes" >> ${LOG_FILE};
        elif [ "$result" = "403" ]; then
        echo -n -e "\r Cygame ウマ娘 プリティーダービー:\t${Font_Red}No${Font_Suffix}\n" && echo -e " Cygame ウマ娘 プリティーダービー:\tNo" >> ${LOG_FILE};
    else
        echo -n -e "\r Cygame ウマ娘 プリティーダービー:\t${Font_Red}Failed (Unexpected Result: $result)${Font_Suffix}\n" && echo -e " Cygame ウマ娘 プリティーダービー:\tFailed (Unexpected Result: $result)" >> ${LOG_FILE};
    fi
}
...
function MediaUnlockTest_DMM() {
    echo -n -e "DMM :\t\t\t\t\t";
    # 测试，连续请求两次 (单独请求一次可能会返回35, 第二次开始变成0)
    local result=`curl --user-agent "${UA_Dalvik}" -${1} -fsL --write-out %{http_code} --output /dev/null --max-time 30 https://api.notification.dmm.com/`;
    if [ "$result" = "000" ]; then
        echo -n -e "\r\r DMM :\t\t\t\t\t${Font_Red}Failed (Network Connection)${Font_Suffix}\n" && echo -e "\r DMM :\t\t\t\t\tFailed (Network Connection)" >> ${LOG_FILE};
        elif [ "$result" = "500" ]; then
        echo -n -e "\r\r DMM :\t\t\t\t\t${Font_Green}Yes${Font_Suffix}\n" && echo -e "\r DMM :\t\t\t\t\tYes" >> ${LOG_FILE};
        elif [ "$result" = "403" ]; then
        echo -n -e "\r\r DMM :\t\t\t\t\t${Font_Red}No${Font_Suffix}\n" && echo -e "\r DMM :\t\t\t\t\tNo" >> ${LOG_FILE};
    else
        echo -n -e "\r\r DMM :\t\t\t\t\t${Font_Red}Failed (Unexpected Result: $result)${Font_Suffix}\n" && echo -e "\r DMM :\t\t\t\t\tFailed (Unexpected Result: $result)" >> ${LOG_FILE};
    fi
}

function MediaUnlockTest_Kancolle() {
    echo -n -e " 艦隊これくしょん-艦これ-:\t\t->\c";
    # 测试，连续请求两次 (单独请求一次可能会返回35, 第二次开始变成0)
    ...
    if [ "$result" = "000" ]; then
        echo -n -e "\r 艦隊これくしょん -艦これ-:\t\t${Font_Red}Failed (Network Connection)${Font_Suffix}\n" && echo -e " 艦隊これくしょん -艦これ-:\t\tFailed (Network Connection)" >> ${LOG_FILE};
        elif [ "$result" = "200" ]; then
        echo -n -e "\r 艦隊これくしょん -艦これ-:\t\t${Font_Green}Yes${Font_Suffix}\n" && echo -e " 艦隊これくしょん -艦これ-:\t\tYes" >> ${LOG_FILE};
        elif [ "$result" = "403" ]; then
        echo -n -e "\r 艦隊これくしょん -艦これ-:\t\t${Font_Red}No${Font_Suffix}\n" && echo -e " 艦隊これくしょん -艦これ-:\t\tNo" >> ${LOG_FILE};
    else
        echo -n -e "\r 艦隊これくしょん -艦これ-:\t\t${Font_Red}Failed (Unexpected Result: $result)${Font_Suffix}\n" && echo -e " 艦隊これくしょん -艦これ-:\t\tFailed (Unexpected Result: $result)" >> ${LOG_FILE};
    fi
}
...
check6=`2606:4700:4700::1111 -c 1 2>&1`;
