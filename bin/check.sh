...
function MediaUnlockTest_PCRTW() {
    ...
    if [ "$result" = "000" ]; then
        echo -n -e "\r So-net プリンセスコネクト！Re:Dive:\t${Font_Red}Failed (Network Connection)${Font_Suffix}\n" && echo -e " So-net プリンセスコネクト！Re:Dive:\tFailed (Network Connection)" >> ${LOG_FILE};
        elif [ "$result" = "404" ]; then
        echo -n -e "\r So-net プリンセスコネクト！Re:Dive:\t${Font_Green}Yes${Font_Suffix}\n" && echo -e " So-net プリンセスコネクト！Re:Dive:\tYes" >> ${LOG_FILE};
        elif [ "$result" = "403" ]; then
        echo -n -e "\r So-net プリンセスコネクト！Re:Dive:\t${Font_Red}No${Font_Suffix}\n" && echo -e " So-net プリンセスコネクト！Re:Dive:\tNo" >> ${LOG_FILE};
    else
        echo -n -e "\r So-net プリンセスコネクト！Re:Dive:\t${Font_Red}Failed (Unexpected Result: $result)${Font_Suffix}\n" && echo -e " So-net プリンセスコネクト！Re:Dive:\tFailed (Unexpected Result: $result)" >> ${LOG_FILE};
    fi
}
